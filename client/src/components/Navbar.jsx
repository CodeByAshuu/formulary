    ...
    <nav className="sticky top-0 z-60 w-full bg-surface/80 backdrop-blur-md border-b border-outline-variant/30 px-6 py-3 flex items-center justify-between font-body transition-all duration-300">
      <div className="flex items-center gap-10">
        <Link to="/" className="flex items-center gap-2.5 group">
          <div className="w-9 h-9 rounded-full bg-linear-to-br from-primary to-primary-container flex items-center justify-center shadow-lg transition-transform group-hover:scale-110">
            <span className="material-symbols-outlined text-white text-xl">clinical_notes</span>
          </div>
          <span className="font-display text-lg tracking-widest text-primary transition-colors group-hover:text-secondary">
            Formulary
          </span>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              to={link.path}
              className={`text-xs font-medium uppercase tracking-widest transition-all duration-300 hover:text-primary ${
